# 現時点でのまとめ

第五章で扱った非正規化や、第二章で扱ったディスクI/Oの分散(RAID)以外にもパフォーマンスを向上させる施策(**パフォーマンス設計**)はある。

それは『**インデックス**』と『**統計情報**』である。

---

# 統計情報について

**インデックス**とは書籍における**索引**(各種単語とそれ等が載っているページ番号からなるもの)のようなものであり、

『**求めたいデータとそのデータがあるメモリアドレス**』といった『**キー(求めたいデータ)と値(ポインタ)**』から成る**map構造をしたオブジェクト**である。

このオブジェクトは『**DBMS内にテーブルとは独立して保持される**』といった特徴がある。

---

**統計情報**とは**求めたいデータに到達するための最短の経路情報**をはじきだす地図情報のことであり、ちょうどドライバーに対する**カーナビ**のようなものである。

以前はエンジニアがある程度この部分をコントロールすることが多かった(**ルールベース**)が、今はDBMSに一任するアーキテクチャ(**コストベース**)が主流である。

---

インデックスには、

- アプリケーションのコードに影響を与えない(**アプリケーション透過的**)

- テーブルにもそこにあるデータにも影響を与えない(**データ透過的**)

- それでいて性能改善の効果が大きい(**インデックスの性能は、データ量に対して線形よりも緩くしか劣化しない**)

上記のような基本要素がある。

---

インデックスにも色々な種類があるが、よく使われるのが『**B-treeインデックス**』であり、あるカラムにインデックスを貼る際は通常DBMSはこのインデックスを貼る。

B-treeインデックスは欲しいデータに対する**木構造**になっていて、**ルートノードからリーフノード、リーフノードが持つポインタが指し示すデータ、といった構造**をしている。

インデックスが貼られたカラムは**ソート化**されるので、DBMS内部でソート化される処理(**集計関数**、**集合関数**、**OLAP関数**、**group by**、**order by**等)を高速化させることができる。

- **集計関数**...`count`, `sum`, `avg`, `max`, `min`

- **集合関数**...`union`, `intersect`, `except`

- **OLAP関数**...`rank`, `row_number`など

上記のような特徴を持つB-treeインデックスにおいては、以下のようなテーブルのカラムにインデックスを貼ることが有効である。

- **レコード数が1万を超える**テーブルのカラム

- **カーディナリティ**(この場合のカーディナリティとは、**あるカラムにあるデータが多様である様**)が高いカラム

*ただし、そのデータが平均的に分散していて、ある特定のデータに偏って登録されていないこと*を条件とする

- **WHERE句の選択条件、結合条件**に使用されているカラム

**選択条件**、**結合条件**については以下の記事を参照。
https://blogs.itmedia.co.jp/doc-consul/2015/11/sql.html

選択条件とは**WHERE句の対象カラムのこと**、つまりは**絞り込む条件**のように思われ、結合条件とは**テーブル結合時の外部キーのこと**と思われる。

# 以下のようなカラムにはインデックスは貼れない！

こういった選択条件や結合条件にインデックスを貼る際、**以下のような場合にはインデックスを貼ることは出来ないので注意！**

**⭐️インデックスを作成したカラムはSQLにおいて『裸』で用いること！**

*`col_1`というカラムにインデックスを貼っているのであって、`col_1 * 1.1`や`substr(col_1, 1, 1)`に貼っているのではない！*

- インデックスを貼ったカラムに演算を行っている。

例えば...

(`where col_1 * 1.1 > 100 `)

こんなクエリにはインデックスは貼れない!

上記は、

(`where col_1 > 100/1.1 `)

のように書き換えること！

- 索引カラムにSQL関数を使っている

例：　

(`where substr(col_1, 1, 1) = 'a' `)

- `IS NULL述語`を使っている

- `否定形`を使っている

- `OR`を使っている

例えば...

(`where col_1 = 99 or col_1 = 100 `)

こんなクエリにはインデックスは貼れない!

上記は、

(`where col_1 in(99, 100) `)

のように書き換えること！

- **後方一致**、または**中間一致**の`LIKE述語`を使っている

- 暗黙の型変換を行っている

例えば`col_1`というカラムが`文字列型`であったとして、

(`where col_1 = 10 `)

という処理はDBMSが内部で暗黙の型変換を行うことでエラーを起こさないようにしているが、この場合はインデックスが使用されなくなる。

なので**明示的に型変換**をしてあげること！

---

# 統計情報について

**統計情報**においては以下のような経路で考える。

1. ユーザーが書いたクエリを**パーサー**(parse 解析する)が解析して、構文的に間違っているか調べる

2. 上記で問題がなかったクエリを**オプティマイザー**(optimize 最適化する)が受け取る

3. オプティマイザーがこのクエリを**カタログマネージャー**(図書館の司書のように欲しいデータに対する経路情報を色々と提示してくれるモジュール)が受け取り、欲しいデータに対する経路情報を色々と提示してくれる

4. 上記で提示された経路情報のうち、オプティマイザーが最も最短な経路情報と判断したものを統計情報とする

5. この統計情報を用いてデータに対する最短処理となるような**実行計画**(処理手順)を立てて、その実行計画のもとにクエリを手続き的に実行してテーブルやインデックスのデータにアクセスする

--- 

このように統計情報を扱うが、

- **更新頻度が多いテーブル(インデックス)**

上記に関しては**テーブルやインデックス作成時と比較してデータの量や分散具合も変化しているので、統計情報を再度集め直す必要がある。**

そうしないと、**カーナビ的に言えば古い地図情報をもとにした経路情報しか示さないので...**

ただ集め直すにしても**それは出来るだけ早い時期で、かつ夜間にやることが大事。**

(このようなテーブル・インデックスから統計情報を集め直す処理はとても重いので)

また全てのテーブル・インデックスを対象にするのではなく、**影響を受けてそうなテーブル・インデックスに絞って集め直すことも大事。**

加えて、上記のように統計情報の更新をするのではなく『**統計情報を凍結する**』という方法もある。

これは**システムやアプリのサービス終了時点でのデータ量やデータの分散具合をあらかじめ予想して、**

**その時点での統計情報を作っておくことで、これ以上パフォーマンスが悪化することがないようにする方法**のことを言う。

ただこれは**オプティマイザーの機能を悲観的に見積もった場合の方法であり、かつこれを行うための事前準備なども大変なので、あまり行われる方法ではない。**
