...
推移的関数従属の説明が間違っていた！
↓
主キーだけではBカラムの値を特定できず、主キーからAカラムの値を特定し、Aカラムの値からはじめてBカラムの値が特定される場合、この関数従属を**推移的関数従属**という
↓
故に第三正規化のためには、元のテーブルからAカラム(主キーにさせる)とBカラムからなるテーブルを切り出し、元のテーブルにはAカラムを外部キー化したカラムを作り、完全関数従属させたテーブルを作ること！ 
↓
噛み砕いて言うと、「**芋づる式でないとデータを特定できない状況を無くすこと**」が推移的関数従属を無くすこと！